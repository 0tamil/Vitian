...
public final class WindowMetricsCalculatorCompat implements WindowMetricsCalculator {
    ...
    public WindowMetrics computeCurrentWindowMetrics(Activity activity) {
        ...
        return new WindowMetrics(i2 >= 30 ? ActivityCompatHelperApi30.INSTANCE.currentWindowBounds(activity) : i2 >= 29 ? computeWindowBoundsQ$window_release(activity) : i2 >= 28 ? computeWindowBoundsP$window_release(activity) : i2 >= 24 ? computeWindowBoundsN$window_release(activity) : computeWindowBoundsIceCreamSandwich$window_release(activity));
    }
    ...
    public final Rect computeWindowBoundsP$window_release(Activity activity) {
        ...
        if (!activityCompatHelperApi24.isInMultiWindowMode(activity)) {
            ...
            if (i2 + navigationBarHeight == point.y) {
                rect2.bottom = i2 + navigationBarHeight;
            } else {
                int i3 = rect2.right;
                if (i3 + navigationBarHeight == point.x) {
                    rect2.right = i3 + navigationBarHeight;
